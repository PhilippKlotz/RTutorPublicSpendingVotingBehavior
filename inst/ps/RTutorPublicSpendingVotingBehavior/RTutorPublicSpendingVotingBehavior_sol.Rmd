...
library(RTutor)
setwd("C:/Users/klotz/OneDrive - bwedu/6. Semester/Bachelorarbeit/Ausarbeitung")
ps.name = "RTutorPublicSpendingVotingBehavior"; sol.file = paste0(ps.name,"_sol.Rmd")
libs = c("ggplot2", "dplyr", "stargazer", "lfe", "tidyr", "sf") # character vector of all packages you load in the problem set
#
create.ps(sol.file=sol.file, ps.name=ps.name, libs=libs,addons = "quiz", stop.when.finished= FALSE)

# The following line directly shows the problem set
# in the browser
# show.ps(ps.name,launch.browser=TRUE,
#         auto.save.code=FALSE,sample.solution=FALSE, clear.user = TRUE)
...
rtutor.package.skel(sol.file=sol.file, ps.name=ps.name,libs=libs,
                    pkg.name="RTutorPublicSpendingVotingBehavior",   # Name of the problem set package
                    pkg.parent.dir = "C:/Users/klotz/OneDrive - bwedu/6. Semester/Bachelorarbeit/RTutorPublicSpendingVotingBehavior", # Parent directory
                    author="Philipp Klotz", # Your name
                    github.user="PhilippKlotz",     # Your github user name
...
#ps.name  =  "ausarbeitung"  # Name des Problemsatzes
#libs  =  c ("ggplot2", "dplyr", "stargazer", "lfe", "tidyr") # Liste aller Bibliotheken, die von ps verwendet werden
# Ordner, der Ihre .rps-Datei enthält
# rps.dir  =  "C:/Users/klotz/OneDrive - bwedu/6. Semester/Bachelorarbeit/RTutorPublicSpendingVotingBehavior/RTutorPublicSpendingVotingBehavior/inst/ps/ausarbeitung"
# # Ordner, in dem die App erstellt werden soll
# app.dir  =  "C:/Users/klotz/OneDrive - bwedu/6. Semester/Bachelorarbeit/shinyapps.io"
# rtutor.app.skel (ps.name= ps.name , app.name= "RTutorPublicSpendingVotingBehavior" ,
...
#< info "Public goods"
What exactly are public goods? Public goods are characterized by two aspects: **non-rivalry** and **non-excludability**  
Non-rivalry, on the one hand, means that the consumption of one economic unit does not affect the consumption of another one. The term non-excludability on the other hand means that it is difficult to exclude someone from the consumption. For a purely public good, both of these conditions must be met.  
The extent of public goods can be local, national, or even global. A common example of a public good is the national defense. It can be characterized as a public good since it is not rival and there is hardly any possibility to exclude a single individual from it.  
One can show that in the reality public goods will be under-supplied if left only to the public sector. For this reason, they should be provided by the state and financed with resources that were gained by taxes.
If you are interested in more detailed information about public goods you can look it up <a href="https://www.econlib.org/library/Enc/PublicGoods.html" target="_blank">here</a>.
#>
...
#< hint
display("Use Google, e.g. search for 'R read RDS file'.")
#>
```
...
#< quiz "dplyr"
question: What do you think, which command suits best to choose only certain variables?
sc:
    - select()*
    - summarize()
    - group_by()
...
library(dplyr)
ARRA_central = ARRA %>%
  select(c(Municipality, InputID, year, Distance, treated, demo_share))
head(ARRA_central)
```
...
#< quiz "To be treated or not to be."
question: What do you think the variable treatment tells us?
sc:
    - Whether a municipality is a township.
    - Whether a municipality has more the 5000 residents.
    - Whether the distance to an ARRA project is relatively low.*
    - Whether there was an election.
success: Correct!
failure: Try again.
#>
...
#< quiz "Comparison of sizes"
question: Take a look at the summary of the column `treated` that we have created above. Which group do you think contains more observations, the treatment or the control group?
sc:
    - The treatment group is larger
    - The control group is larger*
    - We can't answer the question based on our data.
...
#< award "The first steps"
Congratulations, you have successfully read in the data set and made yourself a first overview of the data. This is essential for any further analyses.
#>
...
#< hint
display("If you don't know what command to use try readRDS().")
#>
```
...
A central aspect of all common programming languages are data types. These are used to store different types of data appropriately in variables so that afterward one can continue working with them. As in Python, Java, C, and many other programming languages, there are also various data types in R. We distinguish between the following six basic data types:  

**logical:** Also known as *boolean*. This logical value can only take the values true or false.  
**numeric:** This data type represents all real numbers either with or without decimals.  
**integer:** Integer represents whole numbers. This data type cannot take decimals.  
**complex:** In contrast to the previous types, a complex data type can represent imaginary values. The suffix i indicates the imaginary part, e.g. 2 + 5i.  
**character:** A character is usually used to store a single letter or a string.  
**raw:** This is a rather unusual data type since it stores values as raw bits.  

One advantage of R is that, unlike many other programming languages, it is not necessary to declare a variable at the beginning. Rather the variables are assigned with R-objects whereby the data type of this R-object will become the data type of the variable.
...
#>
# sample solution
map$CENSUS2010 = as.numeric(map$CENSUS2010)
...
#>
# sample solution
mapping_data = left_join(map, ARRA_single, by=c("CENSUS2010"="InputID"))
head(mapping_data)
```
...
The most common and popular functions in the join family beside the `left_join()` are the `right_join()`, `inner_join()` and the `full_join()`.  
Equivalent to the `left_join()` the `right_join()` is also used, if we need all rows from one data set (in this case the one from the right one) and only the matched rows from the other one in our new data set. If you are clear about this concept, you will come to the conclusion that you can use both the left and the right join for the same problem. In this case, you would only have to change the order of the parameters, i.e. the data sets in the function.    
The result of the `inner_join()` contains all columns of the data sets x and y and all the rows of x and y for which R finds matching values. As you may imagine the `full_join()`, which is also called outer join, contains not only the matching but all values from x and y. Missing values will create NAs.  
If you want to learn even more about this topic you can click <a href="https://www.datasciencemadesimple.com/join-in-r-merge-in-r/" target="_blank">here</a>.
...
#< award "Map master and erroneous data."
Great. We just worked out a quite challenging plot using `ggplot()`. In addition you learned something about data types in R and how to deal with erroneous values.
#>
...
exp = ARRA %>%
  filter(year2012e == 1) %>%
  group_by(treated) %>%
  summarize(y_exp = mean(demo_share))
head(exp)
```
...
```{r "3 a) 2"}
y_exp_tr = exp$y_exp[2]
y_exp_co = exp$y_exp[1]
```
...
```{r "3 a) 3"}
simple_diff = round(y_exp_tr - y_exp_co, 2) 
simple_diff
```
...
ARRA_summary = ARRA %>%
  group_by(year, group) %>%
  summarize(mean_demo_share = mean(demo_share))
ARRA_summary
```
...
`ggplot()` from the `ggplot2` package is a central function in R and one of the most important when it comes to the appealing graphical presentation of data. In a previous task, we already used this function to plot a map of New Jersey. Besides this application field, `ggplot()` also allows us to create more conventional graphics. These include line plots (`geom_line()`), histograms (`geom_histogram()`), box plots (`geom_boxplot()`) and many more. Even if the structure seems rather complex and confusing at first glance, you will get used to it quite quickly once you have worked with it for a while. The advantage of this function is, that it gives you an enormous number of possibilities to edit your plot in every possible way, select labels, change layouts, and much more.  
If you are interested, you can find more information  <a href="https://datacarpentry.org/R-ecology-lesson/04-visualization-ggplot2.html" target="_blank">here</a>.
...
pre = ARRA %>%
  filter(year2012e == 0) %>%
  group_by(treated) %>%
  summarize(y_pre = mean(demo_share))
head(pre)
```

#< award "Master of dplyr"
Congratulations, you have mastered the most important features of the `dplyr` package including `filter()`, `group_by()`, `summarize()` and `mutate()`.
#>
...
```{r "3 c) 2"}
y_pre_tr = pre$y_pre[2]
y_pre_co = pre$y_pre[1]
```
...
```{r "3 c) 3"}
DiD = round((y_exp_tr - y_exp_co) - (y_pre_tr - y_pre_co), 2)
simple_diff
DiD
```
...
#< quiz "Parallel trends"
question: Is the parallel trend assumption fulfilled for our ARRA project?
sc:
    - Yes it is. The lines run parallel until the experimental period begins.*
    - No, because after the year 2010 the trends do not run parallel anymore.
    - We can't check this assumption based on this plot.
    - No, because the difference between both lines is too small.
success: Well done!
failure: Try again.
#>
...
# sample solution
library(stargazer)
reg2 = felm(demo_share ~ treated_year2012e + treated + year2012e |0|0| InputID, data = ARRA)
stargazer(reg1, reg2, type = "html", title = "Table 1: Estimating the treatment effect via basic regressions", add.lines=list(c('Cluster robust standard errors', 'x','✓')))
```
...
In case you don't have any experience yet in understanding the output of the `stargazer()` function, let's take a quick look at it:  
On the left side, we find the estimated coefficients for all variables we controlled for in the regression (ß<sub>i</sub>) as well as the estimated constant of the model (ß<sub>0</sub>). Below each of these coefficients, `stargazer()` shows the standard error of this estimators in parentheses.  
A special aspect, from which the function also got its name, are the significance stars next to the respective estimators. Here *** means that the estimator is significant at the 1% level and similarly ** indicate significance at the 5% level and * significance at the 10% level. No star means that an estimated coefficient is not significant at any relevant level. This information can also be obtained from the "Note".  
One feature I added are the lines indicating what else we include in our models. In the further course of the problem set there will be lines showing whether or not we included cluster robust standard errors, fixed effects, and further control variables.
The other figures are not relevant for our purposes. If you are still interested, you can find more information in the explanation of the summary output which I linked above. Just click <a href="https://towardsdatascience.com/understanding-linear-regression-output-in-r-7a9cbda948b3" target="_blank">here</a>.

#>

#< quiz "Different standard errors"
question: Compare both columns. What do you notice?
sc:
    - The standard errors for all coefficients decrease.
    - The standard error for treated_year2012e decreases sharply.*
    - The significance of the relevant coefficient remains the same.
    - We estimate slightly different effects.
success: Well done.
failure: Try again.
#>
...
#>
# sample solution
reg3 = felm(demo_share ~ treated_year2012e + treated | I(year*counties) |0| InputID, data = ARRA)
```
...
#>
# sample solution
reg4 = felm(demo_share ~ treated_year2012e + treated + year2012e + blackInterp + hispInterp + unempInterp +  popdensity + RE  + township + borough + city | 0 | 0 | InputID, data = ARRA)
```
...
#>
# sample solution
reg5 = felm(demo_share ~ treated_year2012e + treated + blackInterp + hispInterp + unempInterp + popdensity + RE  + township + borough + city | I(year*counties) | 0 | InputID, data = ARRA)
stargazer(reg1, reg2, reg3, reg4, reg5, type = "html", title = "Table 2: ARRA Public Good Spending and Voting - A binary measure", add.lines=list(c('Cluster robust standard errors', 'x','✓','✓','✓','✓'), c('Fixed effects', 'x','x','✓','x','✓'),c('Demographic controls', 'x','x','x','✓','✓')))
```
...
```{r "4 b) 1"}
ARRA = ARRA %>%
  mutate(distance_year2012e = Distance*year2012e)
```
...
```{r "4 c)"}
sd(ARRA$Distance)*7.403

#< hint
display("The command to calculate the standard deviation is sd()")
#>
```
...
#< quiz "Different Democratic vote shares."
question: Do you know which statement is correct based on how the variables are calculated?
sc:
    - The variable demo_share_ofRD is generally smaller than demo_share.
    - The variable demo_share_ofRD is generally larger than demo_share.*
    - We cannot say in principle which variable is larger or smaller.
    - Both variables should be about the same size.
success: Well done.
failure: Try again.
#>
...
vote_shares = ARRA %>%
  group_by(year) %>%
  summarise(mean_demo_share = mean(demo_share),
            mean_demo_share_ofRD = mean(demo_share_ofRD))
head(vote_shares)
```
...
In general, there are two different formats in which a data set can be available to us. The **long format** and the **wide format**. But what's the difference between these?    
In the wide format, there is a column in which all values are unique. In our case, this is the column `year` which contains each value only once. For all other relevant variables,  there are separate columns. The relevant variables are thus found next to each other, which also explains the name of the format (wide).  
In the long format, on the other hand, the column in which there were previously unique values (`year`) contains the same values multiple times. All the other relevant variables are listed below each other in one single column. That's why the data set is rather long. When converting from wide to long we usually create a new variable that shows which column the values originally came from. Without this, a distinction after the format modification would no longer be possible.  
Of course, there are other ways to convert a data set beside the `gather()` approach we will use in the following. If you are interested in other solutions, you can find them <a href="http://www.cookbook-r.com/Manipulating_data/Converting_data_between_wide_and_long_format/" target="_blank">here</a>.
...
#>
# sample solution
ggplot(data = vote_shares_long, aes(y=mean_demo_share, x=year, color=type)) +
  geom_line(size = 0.7) +
  ylim(45, 60) +
  labs(color = "Legend") + 
  scale_x_continuous(breaks = seq(2000, 2012, by = 4)) +
...
ARRA %>%
  filter(Turnoutpc > 1) %>%
  select(year, pop_total, TotalVote, Turnoutpc)
```
...
#< quiz "Presidential voter turnout"
question: Based on these results, which statement is correct?
sc:
    - The change in the voter turnout is probably relatively large.
    - Most of the estimated relevant coefficients are highly significant.
    - The coefficients of interest are not significant on a relevant level.*
success: Well done.
failure: Try again.
#>
...
#< award "Estimating like an expert"
Great. You just worked with some quite complex regression models to estimate the causal treatment effect. You learned something about different econometric methods including cluster robust standard errors, fixed effects and of course you used different DiD approaches.
#>
...
#< quiz "Derivatives"
question: Take a look at the first derivative. Which statement is correct? You can assume we're considering the ceteris paribus effect, i.e. that the other variables do not change.
sc:
    - As TaxCost increases, the probability of voting Democratic raises.
    - TaxCost has no impact on V.
    - As the TaxCost increases, the probability of voting Democratic decreases.*
success: Perfect!
failure: Try again.
#>
